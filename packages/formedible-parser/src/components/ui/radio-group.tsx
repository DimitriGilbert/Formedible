...
type RadioGroupItemProps = Omit<ComponentProps<typeof RadioPrimitive.Root>, 'className'> & {
  className?: string;
};

function RadioGroup({ className, onValueChange, ...props }: RadioGroupProps) {
  return <RadioGroupPrimitive data-slot="radio-group" className={cn('grid gap-3', className)} onValueChange={(value) => onValueChange?.(value)} {...props} />;
}
