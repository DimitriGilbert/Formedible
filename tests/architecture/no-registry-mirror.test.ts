...
import { assertHasViolations, assertNoViolations, collectRepositoryEntries, type RepositoryEntry } from './utils.js';

export function findRegistryMirrorViolations(entries: readonly RepositoryEntry[]): string[] {
  return entries
...
describe('no registry mirror', () => {
  ...
  it('allows sample shadcn-built registry payload output', () => {
    assertNoViolations(
      findRegistryMirrorViolations([{ relativePath: 'packages/formedible/.compiled/registry/default/form.json', isDirectory: false }]),
    );
  });
