...
// a source-tree scan.
function readDeclaredSourceEntrypointPaths(packageJsonContent, packageName) {
    let parsed;
    try {
...
function findEntrypointStatementViolations(file) {
    ...
    const violations = statements.some((statement) => exportStatementPattern.test(statement))
        ? []
...
describe('package root real exports', () => {
    ...
    it('allows sample packages with real re-exporting source root entrypoints', () => {
        assertNoViolations(findPackageRootExportViolations([
            {
                relativePath: 'packages/formedible/src/index.ts',
