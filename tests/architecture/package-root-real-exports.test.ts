...
export function findPackageRootExportViolations(files: readonly RepositoryFile[]): string[] {
  return files
    ...
    .flatMap((file) => {
      ...
      return invalidStatements.map((statement) =>
        `source root entrypoint must re-export real modules instead of implementing runtime behavior in ${file.relativePath}: ${statement}`,
      );
    });
}
