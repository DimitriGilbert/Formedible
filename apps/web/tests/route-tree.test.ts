...
const expectedRoutes = [
  ...
  { fullPath: '/docs', variableName: 'DocsRouteRouteWithChildren', importAlias: 'DocsRouteRouteImport', source: './routes/docs/route' },
...
describe('docs route tree', () => {
  it('keeps the docs routes registered in generated route tree output', async () => {
    ...
    assert.match(routeTree, /DocsRouteRoute: DocsRouteRouteWithChildren/);
  });

  it('keeps docs hub and docs examples on distinct route files', async () => {
    const routeTree = await readFile(join(process.cwd(), 'src/routeTree.gen.ts'), 'utf8');

    assert.match(routeTree, /import \{ Route as DocsRouteRouteImport \} from '\.\/routes\/docs\/route'/);
    ...
    assert.match(routeTree, /const DocsRouteRoute = DocsRouteRouteImport\.update\(\{\s*id: '\/docs',[\s\S]*path: '\/docs',[\s\S]*getParentRoute: \(\) => rootRouteImport/);
    assert.match(routeTree, /const DocsIndexRoute = DocsIndexRouteImport\.update\(\{\s*id: '\/',[\s\S]*path: '\/',[\s\S]*getParentRoute: \(\) => DocsRouteRoute/);
    assert.match(routeTree, /const DocsExamplesRoute = DocsExamplesRouteImport\.update\(\{\s*id: '\/examples',[\s\S]*path: '\/examples',[\s\S]*getParentRoute: \(\) => DocsRouteRoute/);
