...
export const FieldConfigurator: React.FC<FieldConfiguratorProps> = ({
  ...
  const basicForm = useFormedible({
    ...
    formOptions: {
      ...
      onSubmit: async ({ value }) => {
        onFieldChange(fieldId, { ...initialField, ...value });
      },
    },
  });

  // Options Configuration Form
  const optionsForm = useFormedible({
    schema: optionsConfigSchema,
    fields: [
      { 
        name: "options", 
        type: "array", 
        label: "Options",
        // Array field configuration will be handled differently
      },
    ],
    formOptions: {
      defaultValues: {
  ...
  // Slider Configuration Form
  const sliderForm = useFormedible({
    schema: sliderConfigSchema,
    fields: [
    ...
    formOptions: {
      defaultValues: {
        min: initialField.sliderConfig?.min || 0,
        max: initialField.sliderConfig?.max || 100,
        step: initialField.sliderConfig?.step || 1,
        orientation: initialField.sliderConfig?.orientation || "horizontal",
        showTooltip: initialField.sliderConfig?.showTooltip || false,
        showValue: initialField.sliderConfig?.showValue || false,
      },
      onSubmit: async ({ value }) => {
        onFieldChange(fieldId, { ...initialField, sliderConfig: value });
      },
    },
  });
